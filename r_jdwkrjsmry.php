...
<?php include_once "r_jdwkrjsmryinfo.php" ?>
<?php
...
$r_jdwkrj_summary = NULL; // Initialize page object first

class crr_jdwkrj_summary extends crr_jdwkrj {

	// Page ID
	var $PageID = 'summary';

	// Project ID
	var $ProjectID = "{049CCBC0-CFE5-40D9-8A80-2D734899363E}";

	// Page object name
	var $PageObjName = 'r_jdwkrj_summary';
	...
	function __construct() {
		...
		// Table object (r_jdwkrj)
		if (!isset($GLOBALS["r_jdwkrj"])) {
			$GLOBALS["r_jdwkrj"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["r_jdwkrj"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'r_jdwkrj', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fr_jdwkrjsummary";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'r_jdwkrj');
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fr_jdwkrjsummary\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fr_jdwkrjsummary\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fr_jdwkrjsummary\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
			$rs->MoveFirst(); // Move first
				...
				$this->FirstRowData['tgl_indo'] = ewr_Conv($rs->fields('tgl_indo'), 200);
		...
		if (!$rs->EOF) {
			...
			$this->tgl_indo->setDbValue($rs->fields('tgl_indo'));
			$this->jk_kd->setDbValue($rs->fields('jk_kd'));
			$this->hk_def->setDbValue($rs->fields('hk_def'));
			$this->tgl->setDbValue($rs->fields('tgl'));
			$this->Val[1] = $this->pegawai_nama->CurrentValue;
			$this->Val[2] = $this->tgl_indo->CurrentValue;
			$this->Val[3] = $this->jk_kd->CurrentValue;
			$this->Val[4] = $this->hk_def->CurrentValue;
			$this->Val[5] = $this->tgl->CurrentValue;
		} else {
			$this->pegawai_nama->setDbValue("");
			$this->tgl_indo->setDbValue("");
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			// tgl_indo
			$this->tgl_indo->ViewValue = $this->tgl_indo->CurrentValue;
			$this->tgl_indo->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// tgl_indo
			$CurrentValue = $this->tgl_indo->CurrentValue;
			$ViewValue = &$this->tgl_indo->ViewValue;
			$ViewAttrs = &$this->tgl_indo->ViewAttrs;
			$CellAttrs = &$this->tgl_indo->CellAttrs;
			$HrefValue = &$this->tgl_indo->HrefValue;
			$LinkAttrs = &$this->tgl_indo->LinkAttrs;
			$this->Cell_Rendered($this->tgl_indo, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
	...
	function SetupFieldCount() {
		...
		if ($this->tgl_indo->Visible) $this->DtlColumnCount += 1;
	...
	function GetExtendedFilter() {
		...
		// Reset search command
			...
			if ($this->GetDropDownValue($this->pegawai_nama)) {
				$bSetupFilter = TRUE;
			} elseif ($this->pegawai_nama->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_r_jdwkrj_pegawai_nama'])) {
				$bSetupFilter = TRUE;
			}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_r_jdwkrj_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r_jdwkrj_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_r_jdwkrj_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r_jdwkrj_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_r_jdwkrj_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_r_jdwkrj_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_r_jdwkrj_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_r_jdwkrj_' . $parm] = $sv;
		$_SESSION['so_r_jdwkrj_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_r_jdwkrj_' . $parm] = $sv1;
		$_SESSION['so_r_jdwkrj_' . $parm] = $so1;
		$_SESSION['sc_r_jdwkrj_' . $parm] = $sc;
		$_SESSION['sv2_r_jdwkrj_' . $parm] = $sv2;
		$_SESSION['so2_r_jdwkrj_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_r_jdwkrj_$parm"] = "";
		$_SESSION["rf_r_jdwkrj_$parm"] = "";
		$_SESSION["rt_r_jdwkrj_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->fields($parm);
		$fld->SelectionList = @$_SESSION["sel_r_jdwkrj_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_r_jdwkrj_$parm"];
		$fld->RangeTo = @$_SESSION["rt_r_jdwkrj_$parm"];
	}
	...
	function GetSort($options = array()) {
		...
		// Check for an Order parameter
			...
			$this->UpdateSort($this->tgl_indo, $bCtrl); // tgl_indo
...
// Create page object
if (!isset($r_jdwkrj_summary)) $r_jdwkrj_summary = new crr_jdwkrj_summary();
if (isset($Page)) $OldPage = $Page;
$Page = &$r_jdwkrj_summary;
...
// Create page object
var r_jdwkrj_summary = new ewr_Page("r_jdwkrj_summary");

// Page properties
r_jdwkrj_summary.PageID = "summary"; // Page ID
var EWR_PAGE_ID = r_jdwkrj_summary.PageID;

// Extend page with Chart_Rendering function
r_jdwkrj_summary.Chart_Rendering = 
...
r_jdwkrj_summary.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var CurrentForm = fr_jdwkrjsummary = new ewr_Form("fr_jdwkrjsummary");

// Validate method
fr_jdwkrjsummary.Validate = function() {
...
fr_jdwkrjsummary.Form_CustomValidate = 
...
fr_jdwkrjsummary.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fr_jdwkrjsummary.ValidateRequired = false; // No JavaScript validation
<?php } ?>

// Use Ajax
fr_jdwkrjsummary.Lists["sv_pegawai_nama"] = {"LinkField":"sv_pegawai_nama","Ajax":true,"DisplayFields":["sv_pegawai_nama","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<script type="text/javascript">
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fr_jdwkrjsummary" id="fr_jdwkrjsummary" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fr_jdwkrjsummary_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<input type="text" data-table="r_jdwkrj" data-field="x_tgl" id="sv_tgl" name="sv_tgl" placeholder="<?php echo $Page->tgl->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->tgl->SearchValue) ?>" data-calendar="true" data-formatid="0"<?php echo $Page->tgl->EditAttributes() ?>>
</span>
	<span class="ewSearchCond btw1_tgl"><?php echo $ReportLanguage->Phrase("AND") ?></span>
	<span class="ewSearchField btw1_tgl">
<?php ewr_PrependClass($Page->tgl->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="r_jdwkrj" data-field="x_tgl" id="sv2_tgl" name="sv2_tgl" placeholder="<?php echo $Page->tgl->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->tgl->SearchValue2) ?>" data-calendar="true" data-formatid="0"<?php echo $Page->tgl->EditAttributes() ?>>
</span>
</div>
</div>
...
<script type="text/javascript">
fr_jdwkrjsummary.Init();
fr_jdwkrjsummary.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php include "r_jdwkrjsmrypager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<!-- Report grid (begin) -->
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="pegawai_nama"><div class="r_jdwkrj_pegawai_nama"><span class="ewTableHeaderCaption"><?php echo $Page->pegawai_nama->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="pegawai_nama">
<?php if ($Page->SortUrl($Page->pegawai_nama) == "") { ?>
		<div class="ewTableHeaderBtn r_jdwkrj_pegawai_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->pegawai_nama->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_jdwkrj_pegawai_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->pegawai_nama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->pegawai_nama->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->tgl_indo->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="tgl_indo"><div class="r_jdwkrj_tgl_indo"><span class="ewTableHeaderCaption"><?php echo $Page->tgl_indo->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="tgl_indo">
<?php if ($Page->SortUrl($Page->tgl_indo) == "") { ?>
		<div class="ewTableHeaderBtn r_jdwkrj_tgl_indo">
			<span class="ewTableHeaderCaption"><?php echo $Page->tgl_indo->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_jdwkrj_tgl_indo" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->tgl_indo) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->tgl_indo->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->tgl_indo->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->tgl_indo->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="jk_kd"><div class="r_jdwkrj_jk_kd"><span class="ewTableHeaderCaption"><?php echo $Page->jk_kd->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="jk_kd">
<?php if ($Page->SortUrl($Page->jk_kd) == "") { ?>
		<div class="ewTableHeaderBtn r_jdwkrj_jk_kd">
			<span class="ewTableHeaderCaption"><?php echo $Page->jk_kd->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_jdwkrj_jk_kd" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->jk_kd) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->jk_kd->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="hk_def"><div class="r_jdwkrj_hk_def"><span class="ewTableHeaderCaption"><?php echo $Page->hk_def->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="hk_def">
<?php if ($Page->SortUrl($Page->hk_def) == "") { ?>
		<div class="ewTableHeaderBtn r_jdwkrj_hk_def">
			<span class="ewTableHeaderCaption"><?php echo $Page->hk_def->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_jdwkrj_hk_def" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->hk_def) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->hk_def->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="tgl"><div class="r_jdwkrj_tgl"><span class="ewTableHeaderCaption"><?php echo $Page->tgl->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="tgl">
<?php if ($Page->SortUrl($Page->tgl) == "") { ?>
		<div class="ewTableHeaderBtn r_jdwkrj_tgl">
			<span class="ewTableHeaderCaption"><?php echo $Page->tgl->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_jdwkrj_tgl" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->tgl) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->tgl->FldCaption() ?></span>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_r_jdwkrj_pegawai_nama"<?php echo $Page->pegawai_nama->ViewAttributes() ?>><?php echo $Page->pegawai_nama->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->tgl_indo->Visible) { ?>
		<td data-field="tgl_indo"<?php echo $Page->tgl_indo->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_r_jdwkrj_tgl_indo"<?php echo $Page->tgl_indo->ViewAttributes() ?>><?php echo $Page->tgl_indo->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->jk_kd->Visible) { ?>
		<td data-field="jk_kd"<?php echo $Page->jk_kd->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_r_jdwkrj_jk_kd"<?php echo $Page->jk_kd->ViewAttributes() ?>><?php echo $Page->jk_kd->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->hk_def->Visible) { ?>
		<td data-field="hk_def"<?php echo $Page->hk_def->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_r_jdwkrj_hk_def"<?php echo $Page->hk_def->ViewAttributes() ?>><?php echo $Page->hk_def->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->tgl->Visible) { ?>
		<td data-field="tgl"<?php echo $Page->tgl->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_r_jdwkrj_tgl"<?php echo $Page->tgl->ViewAttributes() ?>><?php echo $Page->tgl->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
