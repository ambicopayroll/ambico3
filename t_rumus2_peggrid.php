...
ft_rumus2_peggrid.Lists["x_pegawai_id"] = {"LinkField":"x_pegawai_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_pegawai_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"pegawai"};
...
?>
<span id="as_x<?php echo $t_rumus2_peg_grid->RowIndex ?>_pegawai_id" style="white-space: nowrap; z-index: <?php echo (9000 - $t_rumus2_peg_grid->RowCnt * 10) ?>">
	<input type="text" name="sv_x<?php echo $t_rumus2_peg_grid->RowIndex ?>_pegawai_id" id="sv_x<?php echo $t_rumus2_peg_grid->RowIndex ?>_pegawai_id" value="<?php echo $t_rumus2_peg->pegawai_id->EditValue ?>" size="30" placeholder="<?php echo ew_HtmlEncode($t_rumus2_peg->pegawai_id->getPlaceHolder()) ?>" data-placeholder="<?php echo ew_HtmlEncode($t_rumus2_peg->pegawai_id->getPlaceHolder()) ?>"<?php echo $t_rumus2_peg->pegawai_id->EditAttributes() ?>>
</span>
<input type="hidden" data-table="t_rumus2_peg" data-field="x_pegawai_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $t_rumus2_peg->pegawai_id->DisplayValueSeparatorAttribute() ?>" name="x<?php echo $t_rumus2_peg_grid->RowIndex ?>_pegawai_id" id="x<?php echo $t_rumus2_peg_grid->RowIndex ?>_pegawai_id" value="<?php echo ew_HtmlEncode($t_rumus2_peg->pegawai_id->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<input type="hidden" name="q_x<?php echo $t_rumus2_peg_grid->RowIndex ?>_pegawai_id" id="q_x<?php echo $t_rumus2_peg_grid->RowIndex ?>_pegawai_id" value="<?php echo $t_rumus2_peg->pegawai_id->LookupFilterQuery(true) ?>">
<script type="text/javascript">
ft_rumus2_peggrid.CreateAutoSuggest({"id":"x<?php echo $t_rumus2_peg_grid->RowIndex ?>_pegawai_id","forceSelect":true});
</script>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($t_rumus2_peg->pegawai_id->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x<?php echo $t_rumus2_peg_grid->RowIndex ?>_pegawai_id',m:0,n:10,srch:false});" class="ewLookupBtn btn btn-default btn-sm"><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" name="s_x<?php echo $t_rumus2_peg_grid->RowIndex ?>_pegawai_id" id="s_x<?php echo $t_rumus2_peg_grid->RowIndex ?>_pegawai_id" value="<?php echo $t_rumus2_peg->pegawai_id->LookupFilterQuery(false) ?>">
</span>
<?php } ?>
