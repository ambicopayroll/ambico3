...
$t_rumus_peg_add = NULL; // Initialize page object first

class ct_rumus_peg_add extends ct_rumus_peg {
	...
	// Page object name
	var $PageObjName = 't_rumus_peg_add';
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("t_rumus_peglist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "t_rumus_pegview.php")
	...
	function AddRow($rsold = NULL) {
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->rumus_peg_id->setDbValue($conn->Insert_ID());
				$rsnew['rumus_peg_id'] = $this->rumus_peg_id->DbValue;
			}
		} else {
...
// Create page object
if (!isset($t_rumus_peg_add)) $t_rumus_peg_add = new ct_rumus_peg_add();

// Page init
$t_rumus_peg_add->Page_Init();

// Page main
$t_rumus_peg_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_rumus_peg_add->Page_Render();
?>
...
var CurrentForm = ft_rumus_pegadd = new ew_Form("ft_rumus_pegadd", "add");

// Validate form
ft_rumus_pegadd.Validate = function() {
...
ft_rumus_pegadd.Form_CustomValidate = 
...
ft_rumus_pegadd.ValidateRequired = true;
<?php } else { ?>
ft_rumus_pegadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_rumus_pegadd.Lists["x_pegawai_id"] = {"LinkField":"x_pegawai_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_pegawai_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"pegawai"};
ft_rumus_pegadd.Lists["x_rumus_id"] = {"LinkField":"x_rumus_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_rumus_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t_rumus"};
...
<?php if (!$t_rumus_peg_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $t_rumus_peg_add->ShowPageHeader(); ?>
<?php
$t_rumus_peg_add->ShowMessage();
?>
<form name="ft_rumus_pegadd" id="ft_rumus_pegadd" class="<?php echo $t_rumus_peg_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_rumus_peg_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_rumus_peg_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ft_rumus_pegadd.CreateAutoSuggest({"id":"x_pegawai_id","forceSelect":true});
</script>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_rumus_peg_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ft_rumus_pegadd.Init();
</script>
<?php
$t_rumus_peg_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_rumus_peg_add->Page_Terminate();
?>
