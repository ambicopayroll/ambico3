	...
	function AddRow($rsold = NULL) {
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$rsnew['rumus_id'] = $this->rumus_id->DbValue;
			}
		} else {
			if ($this->getSuccessMessage() <> "" || $this->getFailureMessage() <> "") {
...
<?php } ?>
<?php if ($t_rumus_list->TotalRecs > 0) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="t_rumus">
...
<?php if ($t_rumus_list->TotalRecs > 0 || $t_rumus->CurrentAction == "add" || $t_rumus->CurrentAction == "copy") { ?>
