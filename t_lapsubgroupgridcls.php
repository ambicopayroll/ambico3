...
$t_lapsubgroup_grid = NULL; // Initialize page object first

class ct_lapsubgroup_grid extends ct_lapsubgroup {
	...
	// Page object name
	var $PageObjName = 't_lapsubgroup_grid';

	// Grid form hidden field names
	var $FormName = 'ft_lapsubgroupgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_lapsubgroup"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->lapsubgroup_id->CurrentValue = strval($arKeys[0]); // lapsubgroup_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "t_lapgroup") {
			$this->lapgroup_id->Visible = FALSE;
			if ($GLOBALS["t_lapgroup"]->EventCancelled) $this->EventCancelled = TRUE;
		}
