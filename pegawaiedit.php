...
<?php if (!isset($pegawai_edit->Pager)) $pegawai_edit->Pager = new cNumericPager($pegawai_edit->StartRec, $pegawai_edit->DisplayRecs, $pegawai_edit->TotalRecs, $pegawai_edit->RecRange) ?>
<?php if ($pegawai_edit->Pager->RecordCount > 0 && $pegawai_edit->Pager->Visible) { ?>
<div class="ewPager">
<div class="ewNumericPage"><ul class="pagination">
	<?php if ($pegawai_edit->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $pegawai_edit->PageUrl() ?>start=<?php echo $pegawai_edit->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($pegawai_edit->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $pegawai_edit->PageUrl() ?>start=<?php echo $pegawai_edit->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($pegawai_edit->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $pegawai_edit->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($pegawai_edit->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $pegawai_edit->PageUrl() ?>start=<?php echo $pegawai_edit->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($pegawai_edit->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $pegawai_edit->PageUrl() ?>start=<?php echo $pegawai_edit->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</div>
<?php } ?>
