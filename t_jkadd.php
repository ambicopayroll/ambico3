...
$t_jk_add = NULL; // Initialize page object first

class ct_jk_add extends ct_jk {
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("t_jklist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "t_jkview.php")
...
// Create page object
if (!isset($t_jk_add)) $t_jk_add = new ct_jk_add();
...
var CurrentForm = ft_jkadd = new ew_Form("ft_jkadd", "add");

// Validate form
ft_jkadd.Validate = function() {
...
ft_jkadd.ValidateRequired = true;
<?php } else { ?>
ft_jkadd.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $t_jk_add->ShowPageHeader(); ?>
<?php
$t_jk_add->ShowMessage();
?>
<form name="ft_jkadd" id="ft_jkadd" class="<?php echo $t_jk_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_jk_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_jk_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_jk_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
