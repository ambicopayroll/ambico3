...
$t_lapsubgroup_view = NULL; // Initialize page object first

class ct_lapsubgroup_view extends ct_lapsubgroup {
	...
	// Page object name
	var $PageObjName = 't_lapsubgroup_view';
	...
	function __construct() {
		...
		if (@$_GET["lapsubgroup_id"] <> "") {
			$this->RecKey["lapsubgroup_id"] = $_GET["lapsubgroup_id"];
			$KeyUrl .= "&amp;lapsubgroup_id=" . urlencode($this->RecKey["lapsubgroup_id"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["lapsubgroup_id"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["lapsubgroup_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			} elseif (@$_POST["lapsubgroup_id"] <> "") {
				$this->lapsubgroup_id->setFormValue($_POST["lapsubgroup_id"]);
				$this->RecKey["lapsubgroup_id"] = $this->lapsubgroup_id->FormValue;
			} else {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "t_lapsubgrouplist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "t_lapsubgrouplist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($t_lapsubgroup_view)) $t_lapsubgroup_view = new ct_lapsubgroup_view();

// Page init
$t_lapsubgroup_view->Page_Init();

// Page main
$t_lapsubgroup_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_lapsubgroup_view->Page_Render();
?>
...
var CurrentForm = ft_lapsubgroupview = new ew_Form("ft_lapsubgroupview", "view");

// Form_CustomValidate event
ft_lapsubgroupview.Form_CustomValidate = 
...
ft_lapsubgroupview.ValidateRequired = true;
<?php } else { ?>
ft_lapsubgroupview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_lapsubgroupview.Lists["x_pembagian2_id"] = {"LinkField":"x_pembagian2_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_pembagian2_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"pembagian2"};
...
<?php if ($t_lapsubgroup->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$t_lapsubgroup_view->IsModal) { ?>
<?php if ($t_lapsubgroup->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $t_lapsubgroup_view->ExportOptions->Render("body") ?>
<?php
	foreach ($t_lapsubgroup_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$t_lapsubgroup_view->IsModal) { ?>
<?php if ($t_lapsubgroup->Export == "") { ?>
...
<?php } ?>
<?php $t_lapsubgroup_view->ShowPageHeader(); ?>
<?php
$t_lapsubgroup_view->ShowMessage();
?>
<?php if (!$t_lapsubgroup_view->IsModal) { ?>
<?php if ($t_lapsubgroup->Export == "") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($t_lapsubgroup_view->Pager)) $t_lapsubgroup_view->Pager = new cPrevNextPager($t_lapsubgroup_view->StartRec, $t_lapsubgroup_view->DisplayRecs, $t_lapsubgroup_view->TotalRecs) ?>
<?php if ($t_lapsubgroup_view->Pager->RecordCount > 0 && $t_lapsubgroup_view->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($t_lapsubgroup_view->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $t_lapsubgroup_view->PageUrl() ?>start=<?php echo $t_lapsubgroup_view->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($t_lapsubgroup_view->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $t_lapsubgroup_view->PageUrl() ?>start=<?php echo $t_lapsubgroup_view->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $t_lapsubgroup_view->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($t_lapsubgroup_view->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $t_lapsubgroup_view->PageUrl() ?>start=<?php echo $t_lapsubgroup_view->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($t_lapsubgroup_view->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $t_lapsubgroup_view->PageUrl() ?>start=<?php echo $t_lapsubgroup_view->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $t_lapsubgroup_view->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<?php } ?>
<form name="ft_lapsubgroupview" id="ft_lapsubgroupview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_lapsubgroup_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_lapsubgroup_view->Token ?>">
<?php } ?>
...
<?php if ($t_lapsubgroup->pembagian2_id->Visible) { // pembagian2_id ?>
	<tr id="r_pembagian2_id">
		<td><span id="elh_t_lapsubgroup_pembagian2_id"><?php echo $t_lapsubgroup->pembagian2_id->FldCaption() ?></span></td>
...
<?php echo $t_lapsubgroup->pembagian2_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($t_lapsubgroup->lapsubgroup_index->Visible) { // lapsubgroup_index ?>
	<tr id="r_lapsubgroup_index">
		<td><span id="elh_t_lapsubgroup_lapsubgroup_index"><?php echo $t_lapsubgroup->lapsubgroup_index->FldCaption() ?></span></td>
...
<?php echo $t_lapsubgroup->lapsubgroup_index->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($t_lapsubgroup->lapgroup_id->Visible) { // lapgroup_id ?>
	<tr id="r_lapgroup_id">
		<td><span id="elh_t_lapsubgroup_lapgroup_id"><?php echo $t_lapsubgroup->lapgroup_id->FldCaption() ?></span></td>
...
<?php echo $t_lapsubgroup->lapgroup_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($t_lapsubgroup->Export == "") { ?>
<script type="text/javascript">
ft_lapsubgroupview.Init();
</script>
<?php } ?>
<?php
$t_lapsubgroup_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($t_lapsubgroup->Export == "") { ?>
...
<?php
$t_lapsubgroup_view->Page_Terminate();
?>
