...
//		$item->Visible = TRUE;
	...
	function SetupPopup() {
		...
		// Get 'reset' command
			...
			if (strtolower($sCmd) == "reset") {
				$_SESSION["sel_r_rekon_tgl"] = "";
				$_SESSION["rf_r_rekon_tgl"] = "";
				$_SESSION["rt_r_rekon_tgl"] = "";
				$this->ResetPager();
			}
		}

		// Load selection criteria to array
		if (is_array(@$_SESSION["sel_r_rekon_tgl"])) {
			$this->tgl->SelectionList = @$_SESSION["sel_r_rekon_tgl"];
			$this->tgl->RangeFrom = @$_SESSION["rf_r_rekon_tgl"];
			$this->tgl->RangeTo = @$_SESSION["rt_r_rekon_tgl"];
		} elseif (@$_SESSION["sel_r_rekon_tgl"] == EWR_INIT_VALUE) { // Select all
			$this->tgl->SelectionList = "";
		}
	}
	...
	function GetExtendedFilter() {
		...
		// Reset search command
			...
			if ($this->GetDropDownValue($this->pegawai_nama)) {
				$bSetupFilter = TRUE;
			} elseif ($this->pegawai_nama->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_r_rekon_pegawai_nama'])) {
				$bSetupFilter = TRUE;
			}
	...
	function ValidateForm() {
		...
		if (!EWR_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!ewr_CheckDate($this->tgl->SearchValue)) {
			if ($gsFormError <> "") $gsFormError .= "<br>";
			$gsFormError .= $this->tgl->FldErrMsg();
		}
		if (!ewr_CheckDate($this->tgl->SearchValue2)) {
			if ($gsFormError <> "") $gsFormError .= "<br>";
			$gsFormError .= $this->tgl->FldErrMsg();
		}
...
// Use Ajax
fr_rekoncrosstab.Lists["sv_pegawai_nama"] = {"LinkField":"sv_pegawai_nama","Ajax":true,"DisplayFields":["sv_pegawai_nama","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<script type="text/javascript">
...
<input type="text" data-table="r_rekon" data-field="x_tgl" id="sv_tgl" name="sv_tgl" placeholder="<?php echo $Page->tgl->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->tgl->SearchValue) ?>" data-calendar="true" data-formatid="5"<?php echo $Page->tgl->EditAttributes() ?>>
</span>
	<span class="ewSearchCond btw1_tgl"><?php echo $ReportLanguage->Phrase("AND") ?></span>
	<span class="ewSearchField btw1_tgl">
<?php ewr_PrependClass($Page->tgl->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="r_rekon" data-field="x_tgl" id="sv2_tgl" name="sv2_tgl" placeholder="<?php echo $Page->tgl->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->tgl->SearchValue2) ?>" data-calendar="true" data-formatid="5"<?php echo $Page->tgl->EditAttributes() ?>>
</span>
</div>
</div>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="pegawai_nama">
		<div class="r_rekon_pegawai_nama"><span class="ewTableHeaderCaption"><?php echo $Page->pegawai_nama->FldCaption() ?></span></div>
	</td>
<?php } else { ?>
	<td data-field="pegawai_nama">
<?php if ($Page->SortUrl($Page->pegawai_nama) == "") { ?>
		<div class="ewTableHeaderBtn r_rekon_pegawai_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->pegawai_nama->FldCaption() ?></span>			
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_rekon_pegawai_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->pegawai_nama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->pegawai_nama->FldCaption() ?></span>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_r_rekon_pegawai_nama"<?php echo $Page->pegawai_nama->ViewAttributes() ?>><?php echo $Page->pegawai_nama->GroupViewValue ?></span></td>
<?php } ?>
<!-- Dynamic columns begin -->
<?php
