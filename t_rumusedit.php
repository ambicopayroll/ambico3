...
class ct_rumus_edit extends ct_rumus {
	...
	function RenderRow() {
		...
		$this->lembur->ViewCustomAttributes = "";
			...
			// hk_gol
			$this->hk_gol->EditAttrs["class"] = "form-control";
			$this->hk_gol->EditCustomAttributes = "";
			$this->hk_gol->EditValue = ew_HtmlEncode($this->hk_gol->CurrentValue);
			$this->hk_gol->PlaceHolder = ew_RemoveHtml($this->hk_gol->FldCaption());
	...
	function ValidateForm() {
		...
		if (!$this->hk_gol->FldIsDetailKey && !is_null($this->hk_gol->FormValue) && $this->hk_gol->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->hk_gol->FldCaption(), $this->hk_gol->ReqErrMsg));
		}
		if (!ew_CheckInteger($this->hk_gol->FormValue)) {
			ew_AddMessage($gsFormError, $this->hk_gol->FldErrMsg());
		}
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($t_rumus->hk_gol->FldErrMsg()) ?>");
...
<?php if (!isset($t_rumus_edit->Pager)) $t_rumus_edit->Pager = new cNumericPager($t_rumus_edit->StartRec, $t_rumus_edit->DisplayRecs, $t_rumus_edit->TotalRecs, $t_rumus_edit->RecRange) ?>
<?php if ($t_rumus_edit->Pager->RecordCount > 0 && $t_rumus_edit->Pager->Visible) { ?>
<div class="ewPager">
<div class="ewNumericPage"><ul class="pagination">
	<?php if ($t_rumus_edit->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $t_rumus_edit->PageUrl() ?>start=<?php echo $t_rumus_edit->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($t_rumus_edit->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $t_rumus_edit->PageUrl() ?>start=<?php echo $t_rumus_edit->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($t_rumus_edit->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $t_rumus_edit->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($t_rumus_edit->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $t_rumus_edit->PageUrl() ?>start=<?php echo $t_rumus_edit->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($t_rumus_edit->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $t_rumus_edit->PageUrl() ?>start=<?php echo $t_rumus_edit->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</div>
<?php } ?>
...
<?php if ($t_rumus->hk_gol->Visible) { // hk_gol ?>
	<div id="r_hk_gol" class="form-group">
		<label id="elh_t_rumus_hk_gol" for="x_hk_gol" class="col-sm-2 control-label ewLabel"><?php echo $t_rumus->hk_gol->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $t_rumus->hk_gol->CellAttributes() ?>>
<span id="el_t_rumus_hk_gol">
<input type="text" data-table="t_rumus" data-field="x_hk_gol" name="x_hk_gol" id="x_hk_gol" size="30" placeholder="<?php echo ew_HtmlEncode($t_rumus->hk_gol->getPlaceHolder()) ?>" value="<?php echo $t_rumus->hk_gol->EditValue ?>"<?php echo $t_rumus->hk_gol->EditAttributes() ?>>
</span>
