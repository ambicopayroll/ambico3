...
//
// Table class for t_rumus_peg
//
class ct_rumus_peg extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 't_rumus_peg';
		$this->TableName = 't_rumus_peg';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t_rumus_peg`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// rumus_peg_id
		$this->rumus_peg_id = new cField('t_rumus_peg', 't_rumus_peg', 'x_rumus_peg_id', 'rumus_peg_id', '`rumus_peg_id`', '`rumus_peg_id`', 3, -1, FALSE, '`rumus_peg_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->rumus_peg_id->Sortable = TRUE; // Allow sort
		$this->rumus_peg_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['rumus_peg_id'] = &$this->rumus_peg_id;

		// pegawai_id
		$this->pegawai_id = new cField('t_rumus_peg', 't_rumus_peg', 'x_pegawai_id', 'pegawai_id', '`pegawai_id`', '`pegawai_id`', 3, -1, FALSE, '`EV__pegawai_id`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t_rumus_peg`";
	}
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT `pegawai_nama` FROM `pegawai` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`pegawai_id` = `t_rumus_peg`.`pegawai_id` LIMIT 1) AS `EV__pegawai_id`, (SELECT `rumus_nama` FROM `t_rumus` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`rumus_id` = `t_rumus_peg`.`rumus_id` LIMIT 1) AS `EV__rumus_id` FROM `t_rumus_peg`" .
			") `EW_TMP_TABLE`";
	...
	function UseVirtualFields() {
		...
		if (strpos($sOrderBy, " " . $this->pegawai_id->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		if ($this->rumus_id->AdvancedSearch->SearchValue <> "" ||
			$this->rumus_id->AdvancedSearch->SearchValue2 <> "" ||
			strpos($sWhere, " " . $this->rumus_id->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		if (strpos($sOrderBy, " " . $this->rumus_id->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		return FALSE;
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$rs['rumus_peg_id'] = $this->rumus_peg_id->DbValue;
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('rumus_peg_id', $rs))
				ew_AddFilter($where, ew_QuotedName('rumus_peg_id', $this->DBID) . '=' . ew_QuotedValue($rs['rumus_peg_id'], $this->rumus_peg_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`rumus_peg_id` = @rumus_peg_id@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->rumus_peg_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@rumus_peg_id@", ew_AdjustSql($this->rumus_peg_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t_rumus_pegview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t_rumus_pegview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t_rumus_pegadd.php?" . $this->UrlParm($parm);
		else
			$url = "t_rumus_pegadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t_rumus_pegedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t_rumus_pegadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t_rumus_pegdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "rumus_peg_id:" . ew_VarToJson($this->rumus_peg_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->rumus_peg_id->CurrentValue)) {
			$sUrl .= "rumus_peg_id=" . urlencode($this->rumus_peg_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["rumus_peg_id"]))
				$arKeys[] = ew_StripSlashes($_POST["rumus_peg_id"]);
			elseif (isset($_GET["rumus_peg_id"]))
				$arKeys[] = ew_StripSlashes($_GET["rumus_peg_id"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->rumus_peg_id->CurrentValue = $key;
