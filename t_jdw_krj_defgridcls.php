...
$t_jdw_krj_def_grid = NULL; // Initialize page object first

class ct_jdw_krj_def_grid extends ct_jdw_krj_def {
	...
	// Page object name
	var $PageObjName = 't_jdw_krj_def_grid';

	// Grid form hidden field names
	var $FormName = 'ft_jdw_krj_defgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_jdw_krj_def"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->jdw_id->CurrentValue = strval($arKeys[0]); // jdw_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
