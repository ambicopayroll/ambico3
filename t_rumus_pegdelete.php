...
$t_rumus_peg_delete = NULL; // Initialize page object first

class ct_rumus_peg_delete extends ct_rumus_peg {
	...
	// Page object name
	var $PageObjName = 't_rumus_peg_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("t_rumus_peglist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in t_rumus_peg class, t_rumus_peginfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("t_rumus_peglist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($t_rumus_peg_delete)) $t_rumus_peg_delete = new ct_rumus_peg_delete();

// Page init
$t_rumus_peg_delete->Page_Init();

// Page main
$t_rumus_peg_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_rumus_peg_delete->Page_Render();
?>
...
var CurrentForm = ft_rumus_pegdelete = new ew_Form("ft_rumus_pegdelete", "delete");

// Form_CustomValidate event
ft_rumus_pegdelete.Form_CustomValidate = 
...
ft_rumus_pegdelete.ValidateRequired = true;
<?php } else { ?>
ft_rumus_pegdelete.ValidateRequired = false; 
<?php } ?>
...
<?php $t_rumus_peg_delete->ShowPageHeader(); ?>
<?php
$t_rumus_peg_delete->ShowMessage();
?>
<form name="ft_rumus_pegdelete" id="ft_rumus_pegdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_rumus_peg_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_rumus_peg_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t_rumus_peg">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($t_rumus_peg_delete->RecKeys as $key) { ?>
...
<?php } ?>
	...
	<tbody>
<?php
$t_rumus_peg_delete->RecCnt = 0;
$i = 0;
while (!$t_rumus_peg_delete->Recordset->EOF) {
	$t_rumus_peg_delete->RecCnt++;
	$t_rumus_peg_delete->RowCnt++;

	// Set row properties
	$t_rumus_peg->ResetAttrs();
	$t_rumus_peg->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$t_rumus_peg_delete->LoadRowValues($t_rumus_peg_delete->Recordset);
...
<?php } ?>
<?php if ($t_rumus_peg->rumus_id->Visible) { // rumus_id ?>
		<td<?php echo $t_rumus_peg->rumus_id->CellAttributes() ?>>
...
<?php
	$t_rumus_peg_delete->Recordset->MoveNext();
}
$t_rumus_peg_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_rumus_peg_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ft_rumus_pegdelete.Init();
</script>
<?php
$t_rumus_peg_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_rumus_peg_delete->Page_Terminate();
?>
