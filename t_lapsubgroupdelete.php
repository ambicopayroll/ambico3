...
$t_lapsubgroup_delete = NULL; // Initialize page object first

class ct_lapsubgroup_delete extends ct_lapsubgroup {
	...
	// Page object name
	var $PageObjName = 't_lapsubgroup_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("t_lapsubgrouplist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in t_lapsubgroup class, t_lapsubgroupinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("t_lapsubgrouplist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($t_lapsubgroup_delete)) $t_lapsubgroup_delete = new ct_lapsubgroup_delete();

// Page init
$t_lapsubgroup_delete->Page_Init();

// Page main
$t_lapsubgroup_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_lapsubgroup_delete->Page_Render();
?>
...
var CurrentForm = ft_lapsubgroupdelete = new ew_Form("ft_lapsubgroupdelete", "delete");

// Form_CustomValidate event
ft_lapsubgroupdelete.Form_CustomValidate = 
...
ft_lapsubgroupdelete.ValidateRequired = true;
<?php } else { ?>
ft_lapsubgroupdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_lapsubgroupdelete.Lists["x_pembagian2_id"] = {"LinkField":"x_pembagian2_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_pembagian2_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"pembagian2"};
...
<?php $t_lapsubgroup_delete->ShowPageHeader(); ?>
<?php
$t_lapsubgroup_delete->ShowMessage();
?>
<form name="ft_lapsubgroupdelete" id="ft_lapsubgroupdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_lapsubgroup_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_lapsubgroup_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t_lapsubgroup">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($t_lapsubgroup_delete->RecKeys as $key) { ?>
...
<?php if ($t_lapsubgroup->pembagian2_id->Visible) { // pembagian2_id ?>
		<th><span id="elh_t_lapsubgroup_pembagian2_id" class="t_lapsubgroup_pembagian2_id"><?php echo $t_lapsubgroup->pembagian2_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_lapsubgroup->lapsubgroup_index->Visible) { // lapsubgroup_index ?>
		<th><span id="elh_t_lapsubgroup_lapsubgroup_index" class="t_lapsubgroup_lapsubgroup_index"><?php echo $t_lapsubgroup->lapsubgroup_index->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_lapsubgroup->lapgroup_id->Visible) { // lapgroup_id ?>
		<th><span id="elh_t_lapsubgroup_lapgroup_id" class="t_lapsubgroup_lapgroup_id"><?php echo $t_lapsubgroup->lapgroup_id->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$t_lapsubgroup_delete->RecCnt = 0;
$i = 0;
while (!$t_lapsubgroup_delete->Recordset->EOF) {
	$t_lapsubgroup_delete->RecCnt++;
	$t_lapsubgroup_delete->RowCnt++;

	// Set row properties
	$t_lapsubgroup->ResetAttrs();
	$t_lapsubgroup->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$t_lapsubgroup_delete->LoadRowValues($t_lapsubgroup_delete->Recordset);

	// Render row
	$t_lapsubgroup_delete->RenderRow();
?>
	<tr<?php echo $t_lapsubgroup->RowAttributes() ?>>
<?php if ($t_lapsubgroup->pembagian2_id->Visible) { // pembagian2_id ?>
		<td<?php echo $t_lapsubgroup->pembagian2_id->CellAttributes() ?>>
<span id="el<?php echo $t_lapsubgroup_delete->RowCnt ?>_t_lapsubgroup_pembagian2_id" class="t_lapsubgroup_pembagian2_id">
...
<?php } ?>
<?php if ($t_lapsubgroup->lapsubgroup_index->Visible) { // lapsubgroup_index ?>
		<td<?php echo $t_lapsubgroup->lapsubgroup_index->CellAttributes() ?>>
<span id="el<?php echo $t_lapsubgroup_delete->RowCnt ?>_t_lapsubgroup_lapsubgroup_index" class="t_lapsubgroup_lapsubgroup_index">
...
<?php } ?>
<?php if ($t_lapsubgroup->lapgroup_id->Visible) { // lapgroup_id ?>
		<td<?php echo $t_lapsubgroup->lapgroup_id->CellAttributes() ?>>
<span id="el<?php echo $t_lapsubgroup_delete->RowCnt ?>_t_lapsubgroup_lapgroup_id" class="t_lapsubgroup_lapgroup_id">
...
<?php
	$t_lapsubgroup_delete->Recordset->MoveNext();
}
$t_lapsubgroup_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_lapsubgroup_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ft_lapsubgroupdelete.Init();
</script>
<?php
$t_lapsubgroup_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_lapsubgroup_delete->Page_Terminate();
?>
